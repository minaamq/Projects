...
void circle(GLfloat center_x, GLfloat center_y , GLfloat radius ,float *colors){
    ...
    glBegin(GL_POLYGON);
        float theta = PI/1000.f;
        for( float angle = 0.0; angle < 2*PI; angle += theta )
...
void Branch(){
        ...
        Rect(195,200,color_Rect,10,200);
        Rect_Line(195,200,color_Line,10,200);

        circle(200,400,20,color_circle);
        circle_Line(200,400,20,color_Line);
}
void Picture(){
    float center[2]={200,400};
    ...
    glPushMatrix();
        ...
        glTranslatef(-center[0],-center[1],0);    //Translate back  to center
            Triangle(100,400,100,color_Triangle_upleft,1);
            Triangle_Line(100,400,100,color_Line,1);
            Triangle(200,400,100,color_Triangle_upright,2);
            Triangle_Line(200,400,100,color_Line,2);
            Triangle(100,300,100,color_Triangle_lowleft,3);
            Triangle_Line(100,300,100,color_Line,3);
            Triangle(200,300,100,color_Triangle_lowright,4);
            Triangle_Line(200,300,100,color_Line,4);
    glPopMatrix();
    Branch();
                               // using double buffer to solve the flashing problem
}
...
void sun(int x, int y)
{
    ...
    {
    th=i*3.1416/180;
    glVertex2f(x+sr*cos(th),y+zr*sin(th));
...
void man()
{
    ...
    if (counterman > 350 && counterman < 400) //get inside schoolschool
        counterman = 1300;
...
void school()
{
    ...
    glVertex2f(420, 380);
    glEnd();
    print(430, 345, 255, 255, 255, "B.I.T College", GLUT_BITMAP_TIMES_ROMAN_24);
...
void introduction()
{
    ...
    glClear(GL_COLOR_BUFFER_BIT);
    print(260, 500, 1, 1, 1, "BANGALORE INSTITUTE OF TECHNOLOGY", GLUT_BITMAP_TIMES_ROMAN_24);
    print(370, 470, 1, 1, 1, "V.V. Puram Road, BANGALORE", GLUT_BITMAP_HELVETICA_18);
...
void keyboard(unsigned char key, int x, int y)
{
    if (key == 'n') {
        ...
        school_dr = 184; school_dg = 0; school_db = 64;
...
void menu(int choice)
{
    ...
    case 1: // Day
        ...
        sr = 47; zr = 47;
        sun(27,27);
        // Set other variables and parameters for the day mode
        break;
    ...
    case 2: // Night
        ...
        sr=0;zr=0;
        sun(0, 0);
        // Set other variables and parameters for the night mode
        break;
